...
> **Phase: D — MVP-1 build (started 2026-05-07).** Phase A + B + C done. **EPIC-003 + EPIC-004 closed.** **EPIC-005 in progress** — Observability + AI Cost. **STORY-019 sub-PR #1 in flight** ships `@starter-saas/observability` foundation: OTel Node SDK + `AdaptiveSampler` (100% errors / 1% healthy per ADR-0006) + Pino logger with PII redactor (18 built-in sensitive-key patterns + adopter extras) + GenAI span helpers (`withGenAiSpan` / `recordGenAiUsage` / `recordPrompt` with OTel GenAI semconv attributes + preview-by-default prompt capture) + `withTenantContext` async-local propagation + `TenantContextSpanProcessor` stamping `tenant.id` on every span. The LLM Gateway integration point for STORY-022 is ready. **313 tests green** across `@starter-saas/{auth, event-bus, saga, tenancy, cli, gateway, starter, notifications, observability}`. STORY-019 sub-PRs #2 (Fastify+Drizzle instrumentation, Pino→OTel Logs Bridge) + #3 (cost-rollup consumer + `platform.llm_call` event) follow; then STORY-020 (cost dashboards + budgets) + STORY-021 (status page) close EPIC-005. **OAuth deferred** (follow-up Story). **All PRs admin-merged per D-57**.
