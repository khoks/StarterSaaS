...
> **Phase: D — MVP-1 build (started 2026-05-07).** Phase A + B + C done. **STORY-032 + STORY-013 + STORY-014 done.** **STORY-015 in progress** — sub-PR #1 landed (`@starter-saas/cli` skeleton + `@starter-saas/tenancy/testing` PGlite harness). **Sub-PR #2 in flight** ships the migration runner: cross-adapter `TenantDb`/`SagaStoreDb` types (postgres-js + pglite), `runTenantMigrations` (5-parallel + continue-on-error/fail-fast + dry-run + idempotent), `DrizzleTenantMigrator` (concrete saga step 3 adapter), `tenant migrate` CLI wiring, **full provisioning saga end-to-end against real Postgres** (first time — closes a STORY-014 deferred AC). **145 tests green** across auth + event-bus + saga + cli + tenancy. **OAuth deferred** (waits for apps/starter HTTP layer). Phase D Story queue continues: STORY-015 sub-PR #3 (archival) → sub-PR #4 (RBAC + middleware) close EPIC-003; then EPIC-004 → EPIC-005 → EPIC-006 → EPIC-007 → EPIC-008. **All PRs admin-merged per D-57**.
