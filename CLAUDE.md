...
> **Phase: B — Grooming (active since 2026-04-27).** No product code yet. MVP-1 surface still pending. **STORY-008 + STORY-010 + STORY-011 done** (32 decisions D-12..D-43; ADRs 0002 + 0003 accepted — full TS stack on AWS+GCP via Pulumi; 10-step deploy CLI + local command portal). **STORY-009 in progress** — architecture grooming with ACs expanded XL to absorb ~14 queued ADRs (ADR-0004 multi-tenancy → ADR-0017 cross-cutting AI / plugin / brand mechanisms). Likely-split planned: STORY-009A core / 9B AI subsystems / 9C plugin + AI mechanisms.
