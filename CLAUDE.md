...
> **Phase: D — MVP-1 build (started 2026-05-07).** Phase A + B + C done. **STORY-032 + STORY-013 done.** **STORY-014 in progress** — sub-PRs #1 + #2 landed. **Sub-PR #3 in flight** ships the 9-step tenant-provisioning saga per ADR-0004 §3 (ProvisioningState + 7 adapter ports w/ Drizzle-backed defaults + 9 step factories with compensation + `runTenantProvisioning` wrapper emitting saga-level `tenant.provisioning_failed` on failure). **100 tests green** across auth + event-bus + saga + tenancy. **OAuth deferred** (waits for apps/starter HTTP layer). Phase D Story queue continues: STORY-014 sub-PR #4 (`withTenants()` + rate-limit) → STORY-015 (migration + archival + RBAC) closes EPIC-003; then EPIC-004 → EPIC-005 → EPIC-006 → EPIC-007 → EPIC-008. **All PRs admin-merged per D-57**.
