...
> **Phase: D — MVP-1 build (started 2026-05-07).** Phase A + B + C done. **EPIC-003 closed**: STORY-032 + STORY-013 + STORY-014 + STORY-015 all done (13 PRs total — auth flows + tenancy schemas + 9-step provisioning saga + cross-schema wrapper + rate-limit middleware + CLI skeleton + pglite harness + migration runner + 2-stage archival + per-tenant RBAC + middleware). **181 tests green** across `@starter-saas/{auth, event-bus, saga, tenancy, cli}`. End-to-end provisioning saga + archival lifecycle + RBAC verified against real Postgres via PGlite. **EPIC-004 picked up** — Communication Plumbing. STORY-016 (Fastify gateway + Zod boundary discipline) is up next; STORY-017 (pg-outbox event-bus adapter) + STORY-018 (email notifications adapter) follow. **OAuth deferred** until STORY-016 lands the HTTP layer. **All PRs admin-merged per D-57**.
