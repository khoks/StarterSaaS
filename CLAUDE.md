...
> **Phase: B — Grooming (active since 2026-04-27).** No product code yet. MVP-1 surface = STORY-012 (in progress, Phase B finale). **STORY-008 + STORY-010 + STORY-011 + STORY-009 done** (44 decisions D-12..D-55; 18 ADRs accepted: ADR-0001 license + ADR-0002 tech stack + ADR-0003 cloud target + ADR-0004 multi-tenancy + ADR-0005 event bus & saga (Kafka-shaped) + ADR-0006 observability (OTel + Langfuse) + ADR-0007 auth (Auth.js) + ADR-0008..0010+0012 AI Subsystems 1/2/3/5 + ADR-0011 LLM Gateway + ADR-0013 plugin spec + AI-validated compat + ADR-0014 AI-assisted upstream merge + ADR-0015 AI-assisted config gen + ADR-0016 ai-ui streaming integration + ADR-0017 status / brand / admin-UI + ADR-0018 deploy portal mechanism). **STORY-012 in progress** — locking the final ~6-8 MVP-1 subsystems from 18+ candidates that emerged through grooming + writing `docs/roadmap/MVP.md` v1 + spinning up subsystem Epics (EPIC-003+).
