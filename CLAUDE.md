...
> **Phase: D — MVP-1 build (started 2026-05-07).** Phase A + B + C done. **EPIC-003 + EPIC-004 closed.** EPIC-004 ships the full communication backbone: STORY-016 (Fastify gateway + apps/starter reference impl, 3 sub-PRs), STORY-017 (PgOutboxEventBus + DLQ replay + saga cancel, 2 sub-PRs), STORY-018 (`@starter-saas/notifications` package — EmailSender contract + ConsoleEmailSender + TemplateRegistry + 4 kit templates + EventBusNotificationsConsumer + saga step 9 adapter + per-tenant overrides + PII scrubbing, 1 PR). **279 tests green** across `@starter-saas/{auth, event-bus, saga, tenancy, cli, gateway, starter, notifications}`. **EPIC-005 (Observability + AI Cost) picked up next** — OTel + GenAI semconv + Langfuse + cost dashboards + per-tenant budget + status page. **OAuth deferred** (follow-up Story). **All PRs admin-merged per D-57**.
