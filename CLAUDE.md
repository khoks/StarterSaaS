...
> **Phase: D — MVP-1 build (started 2026-05-07).** Phase A + B + C done. **STORY-032 + STORY-013 done.** **STORY-014 in progress** — sub-PR #1 landed (`@starter-saas/event-bus` + `@starter-saas/saga` in-memory adapters), **sub-PR #2 in flight** ships `@starter-saas/tenancy` (Drizzle schemas for `platform.{tenants, tenant_migrations, tenant_archive_log, saga_instances}` + Zod contracts + `DrizzleSagaStore` implementing `SagaStore`). **85 tests green** across auth + event-bus + saga + tenancy. **OAuth deferred** (waits for apps/starter HTTP layer). Phase D Story queue continues: STORY-014 sub-PRs #3-#4 (provisioning saga + `withTenants()` + rate-limit) → STORY-015 (migration + archival + RBAC) closes EPIC-003; then EPIC-004 → EPIC-005 → EPIC-006 → EPIC-007 → EPIC-008. **All PRs admin-merged per D-57**.
