...
> **Phase: D — MVP-1 build (started 2026-05-07).** Phase A + B + C done. **STORY-032 + STORY-013 + STORY-014 done.** **STORY-015 in progress** — sub-PRs #1 (CLI skeleton + pglite) + #2 (migration runner + cross-adapter types + saga end-to-end vs real Postgres) landed. **Sub-PR #3 in flight** ships 2-stage archival per ADR-0004 §4: `archiveTenant` (soft, idempotent + audit row) + `restoreTenant` (refuses hard-deleted) + `runHardDeleteSweep` (30d default retention, legal_hold blocks, DROP SCHEMA + audit deleted_at stamp) + `runTenantDoctor` (health + drift detection: orphaned schemas, missing schemas, legal holds, ready-for-delete) + four new CLI subcommands (`tenant archive`/`restore`/`hard-delete`/`doctor`). **163 tests green** across auth + event-bus + saga + cli + tenancy. **OAuth deferred** (waits for apps/starter HTTP layer). Phase D Story queue continues: STORY-015 sub-PR #4 (RBAC + middleware) closes EPIC-003; then EPIC-004 → EPIC-005 → EPIC-006 → EPIC-007 → EPIC-008. **All PRs admin-merged per D-57**.
