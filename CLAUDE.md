...
> **Phase: D — MVP-1 build (started 2026-05-07).** Phase A + B + C done. **STORY-032 + STORY-013 + STORY-014 done.** STORY-014 closed all 4 sub-PRs: event-bus + saga primitives, tenancy schemas + `DrizzleSagaStore`, 9-step provisioning saga with port-based adapters + Drizzle defaults, `withTenants()` cross-schema wrapper + per-tenant rate-limit middleware. **STORY-015 in progress** — `@starter-saas/cli` (per D-42) with `tenant migrate` (5-parallel + continue-on-error per ADR-0004 §1) / `tenant restore` / `tenant doctor` + 2-stage archival + per-tenant RBAC tables + Fastify-shaped `requireRole`/`requirePermission` middleware. **121 tests green** across auth + event-bus + saga + tenancy. **OAuth deferred** (waits for apps/starter HTTP layer). Phase D Story queue continues: STORY-015 closes EPIC-003; then EPIC-004 → EPIC-005 → EPIC-006 → EPIC-007 → EPIC-008. **All PRs admin-merged per D-57**.
