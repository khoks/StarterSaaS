...
> **Phase: D — MVP-1 build (started 2026-05-07).** Phase A + B + C done. **EPIC-003 closed.** **EPIC-004 in progress** — Communication Plumbing. **STORY-016 done** (Fastify gateway + apps/starter reference impl). **STORY-017 done** — 2 sub-PRs landed: `PgOutboxEventBus` production adapter over the Kafka-shaped contract (writer atomic with adopter tx; per-partition serial / cross-partition parallel poller; consumer-group fanout; retry-then-DLQ; in-memory DLQ mirror) + `platform.{outbox, event_dedupe, event_dlq}` schema + `replayDlqEntry` + `cancelSaga` + 4 new CLI subcommands (`events list-dlq`/`events replay`/`sagas list`/`sagas cancel`). pg_notify wakeup + exponential backoff + durable retry counter explicitly deferred to v1+. **239 tests green** across `@starter-saas/{auth, event-bus, saga, tenancy, cli, gateway, starter}`. **STORY-018 (email notifications adapter)** queued next — wires the saga step 9 `NotificationsSender` port to a real provider over the now-working pg-outbox bus; closes EPIC-004. **OAuth deferred** (apps/starter HTTP layer now unblocks it). **All PRs admin-merged per D-57**.
