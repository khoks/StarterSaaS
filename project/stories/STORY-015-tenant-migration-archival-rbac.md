...
- 2026-05-11 — **Sub-PR #2 landed** (PR #35). **Sub-PR #3 in progress**: 2-stage archival + tenant doctor per ADR-0004 §4. New `packages/tenancy/src/archival/` module: `archive.ts` (`archiveTenant(db, input)` — stage 1 soft archive; transaction-wrapped status+archived_at flip + audit row write; idempotent on re-archive) + `restore.ts` (`restoreTenant(db, input)` — reverses soft archive within retention; refuses for hard-deleted/not-found/already-active with discriminated `reason` codes) + `hard-delete.ts` (`runHardDeleteSweep(db, schemaManager, options)` — finds past-retention archived tenants, respects `legal_hold` block, DROPs schemas + records `deleted_at` on the most-recent audit row + flips status to "deleted"; default 30d retention per ADR-0004 §4 side-pick; mock-able clock for tests + dry-run mode) + `doctor.ts` (`runTenantDoctor(db, options)` — health report: all tenants + legal-holds + past-retention candidates + orphaned-schema detection via `information_schema.schemata` comparison + missing-schema detection for status:active/provisioning rows without DB schema) + `types.ts` (result types) + `index.ts`. Added `RestoreTenantInputSchema` Zod contract. Wired four new CLI subcommands: `tenant archive <id>` (--reason, --requesting-user), `tenant restore <id>` (already existed as placeholder; now functional), `tenant hard-delete` (--retention-days, --dry-run), `tenant doctor` (--retention-days). Shared `TenantCliContext` config-loader shape with optional `migrations` + `schemaManager` fields based on the subcommand. **16 new integration tests** against PGlite: archive (flips state + audit row, idempotent re-archive, throws on unknown), restore (success, idempotent on active, not-found, hard-deleted refusal), hard-delete sweep (DROP + status flip + deleted_at stamp; legal-hold blocks; retention-window respected; dry-run lists without dropping), doctor (4 tests including orphaned + missing schemas detection + legal-hold exclusion from ready-for-delete), and a full lifecycle round-trip (archive → restore → archive → hard-delete → restore-refused-after-hard-delete). **Total test count: 163** (48 auth + 10 event-bus + 11 saga + 6 cli + 88 tenancy). Typecheck + build + test green across 9 packages.
